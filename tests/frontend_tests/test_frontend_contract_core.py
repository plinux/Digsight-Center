...
class FrontendCoreContractTest(SourceAssertionsMixin, unittest.TestCase):
  ...
  def test_controller_header_formats_metrics_to_one_decimal_place(self):
    subprocess.run(["node", "tests/frontend_tests/controller_header_behavior.mjs"], check=True)
  ...
  def test_controller_kind_change_refreshes_controller_info_view(self):
    source = self.read_text("assets/js/app.js")
    function_source = self.source_function(source, "handleControllerKindChange")
    bootstrap_source = self.read_text("assets/js/app-bootstrap.js")

    self.assertIn("syncSelectedControllerEndpointInput(elements, appState);", function_source)
    self.assertIn("await syncControllerEndpoint();", function_source)
    self.assertIn("await refreshState();", function_source)
    self.assertIn("const result = await readControllerInfo();", function_source)
    self.assertIn("controllerReadStatusMessage(result)", function_source)
    self.assertIn("controllerReadStatusDetail(result)", function_source)
    self.assertIn("setStatus(formatError(error));", function_source)
    self.assertIn("handleControllerKindChange,", source)
    self.assertIn("handleControllerKindChange,", bootstrap_source)
    self.assertIn("elements.controllerKindSelect.addEventListener(\"change\", handleControllerKindChange);", bootstrap_source)
  ...
  def test_controller_info_uses_four_column_table(self):
    ...
    self.assertIn('title: "连接状态"', source)
    self.assertNotIn('title: "连接"', source)
    self.assertIn("row.append(labelCell, valueCell, nextLabelCell, nextValueCell);", source)
    self.assertIn("if (profile.enabled === false) {", source)
    self.assertIn("const supportsVoltage = Object.prototype.hasOwnProperty.call(profile, \"target_voltage_v\");", source)
    self.assertIn("if (supportsVoltage) {", source)
    self.assertIn("profile.min_target_voltage_v ?? 0", source)
    self.assertIn("const supportsCurrentLimit = Object.prototype.hasOwnProperty.call(profile, \"target_current_limit_ma\");", source)
    self.assertIn("if (supportsCurrentLimit) {", source)
    self.assertIn("renderTrackOutputSettingFields", source)
    self.assertIn("renderTrackOutputSettingFields(form, options.settings || {}, options.settingSpecs || [], disabled, options.defaults?.settings || {})", source)
    self.assertIn("settingSpecs: controllerInfo.track_output_setting_specs || []", source)
    self.assertIn('"data-setting-key": spec.key', source)
    self.assertIn("trackOutputSettingValue(settings, defaultSettings, spec)", source)
    self.assertIn("function trackOutputSettingValue(settings, defaultSettings, spec)", source)
    self.assertIn("if (!isBlankSettingInputValue(configuredValue)) {", source)
    self.assertIn("if (!isBlankSettingInputValue(defaultValue)) {", source)
    self.assertIn("spec.type === \"number\" ? 0 : \"\"", source)
    self.assertIn("title: spec.note || \"\"", source)
    self.assertIn("collectTrackOutputSettings", source)
    self.assertIn("const voltageInput = fieldset.querySelector('[data-field=\"target_voltage_v\"]');", source)
    self.assertIn("const currentInput = fieldset.querySelector('[data-field=\"target_current_limit_ma\"]');", source)
    self.assertIn("if (voltageInput) {", source)
    self.assertIn("if (currentInput) {", source)
    self.assertIn('for (const fieldset of form.querySelectorAll("fieldset[data-mode]"))', source)
    self.assertIn("changes.settings = nextSettings;", source)
    self.assertIn("case \"power_state\":", source)
    self.assertIn("case \"short_circuit_state\":", source)
    self.assertIn("return value ? \"通电\" : \"断电\";", source)
    self.assertIn("return value ? \"发现短路\" : \"无短路\";", source)
    self.assertIn("attrValue === false", helper_source)
    self.assertIn("const deviceWritable = Boolean(controllerInfo.controller_capabilities?.controller_settings);", source)
    self.assertIn("const syncOnMode = Boolean(controllerInfo.controller_capabilities?.profile_settings_on_track_mode);", source)
    self.assertIn("defaults: controllerInfo.default_track_output_settings || {}", source)
    self.assertIn("renderRailComSettings(railComSection, controllerInfo.railcom_setting || {}, handlers);", source)
    self.assertIn("const railComPlus = railComSetting.railcomplus || {};", source)
    self.assertIn("railComPlusInput.disabled = !railComPlus.writable || !input.checked;", source)
    self.assertIn("if (railComPlusInput && railComPlus.available) {", source)
    self.assertIn("settings.railcomplus_enabled = input.checked && railComPlusInput.checked;", source)
    self.assertIn("handlers.onSave?.({settings});", source)
    self.assertIn('"保存"', source)
    self.assertIn('"重置"', source)
    self.assertIn("trackProfileResetPayload", source)
    self.assertIn("default_track_output_settings", source)
    self.assertIn("设置相应的比例会同步调整电压值。", source)
    self.assertIn("当前控制器适配器尚未实现参数写入控制器，保存只会更新本地配置。", source)
    self.assertIn('actions.className = "profile-actions";', source)
    self.assertIn("save.disabled = disabled;", source)
    self.assertIn('"aria-disabled", disabled ? "true" : "false"', source)

  def test_controller_settings_apply_to_device_follows_adapter_capability(self):
    app_source = self.read_text("assets/js/app.js")
    save_source = self.source_function(app_source, "buildControllerSettingsHandlers")
    self.assertIn("const capabilities = appState.controllerInfo?.controller_capabilities || {};", save_source)
    self.assertIn("await syncControllerEndpoint();", save_source)
    self.assertIn("await refreshState();", save_source)
    self.assertIn("const filteredChanges = controllerSettingsChangesForControllerInfo(changes, appState.controllerInfo);", save_source)
    self.assertIn("controllerSettingsShouldApplyToDevice(filteredChanges, capabilities);", save_source)
    self.assertIn("...filteredChanges, apply_to_device: canApplyToDevice", save_source)
    self.assertIn("result.applied_to_device", save_source)
    self.assertIn('"参数已保存到控制器"', save_source)
    self.assertIn('"参数已保存到本地控制器配置"', save_source)
    self.assertIn("function controllerSettingsChangesForControllerInfo(changes, controllerInfo = {})", app_source)
    self.assertIn("if (!railComSetting.railcomplus?.available) {", app_source)
    self.assertIn("delete settings.railcomplus_enabled;", app_source)
    self.assertIn("function controllerSettingsShouldApplyToDevice(changes, capabilities = {})", app_source)
    self.assertIn("if (!capabilities.controller_settings) {", app_source)
    self.assertIn("hasOwn(settings, \"railcom_enabled\") || hasOwn(settings, \"railcomplus_enabled\")", app_source)

  def test_controller_header_disables_unsupported_track_modes(self):
    source = self.read_text("assets/js/controller-view.js")
    self.assertIn("const trackProfiles = controller.track_profiles || state.controllerInfo?.track_profiles || {};", source)
    self.assertIn('const profile = datasetKey === "trackMode" ? trackProfiles[button.dataset.trackMode] : null;', source)
    self.assertIn("button.disabled = disabled;", source)
    self.assertIn('button.setAttribute("aria-disabled", disabled ? "true" : "false");', source)
    self.assertIn("unsupportedModeTitle(button.dataset.trackMode)", source)
    self.assertIn("本控制器不支持DC模式", source)

  def test_track_profiles_use_two_column_grid(self):
    css = self.read_text("assets/css/app.css")
    self.assertIn("grid-template-columns: repeat(2, minmax(0, 1fr));", css)
    self.assertIn("grid-template-columns: minmax(520px, 1fr) minmax(440px, 480px);", css)
